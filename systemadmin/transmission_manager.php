...
if (isset($_POST['lookup_add_btn'])) {
    ...
    if ($name === '') {
        ...
        $errmsg = 'that transmission already exists.';
    } else {
        Lookup::add($table, array('name' => $name, $statusCol => 1));
        $msg = 'Transmission added.';
    }
}
...
if (isset($_POST['lookup_update_btn'])) {
    ...
    if (is_numeric($id) && $name !== '') {
        if (Lookup::nameExists($table, $name, $id)) {
            $errmsg = 'Another transmission already has that name.';
        } else {
            Lookup::update($table, $id, array('name' => $name));
            $msg = 'Transmission updated.';
        }
    }
}
...
if (isset($_POST['lookup_delete_btn'])) {
    ...
    if (is_numeric($id)) {
        ...
        CommonBase::SendRedirect('transmission_manager.php');
        exit;
    }
}
        ...
        <div id="wrapper">
            ...
            <div id="content" class="clearfix">
                <div class="contentwrapper">
                    ...
                    <div class="heading">
                        <h3>Transmission Manager</h3>
                        <ul class="breadcrumb">
                            <li>You are here:</li>
                            <li>Vehicle Attributes</li>
                            <li class="active">Transmission Manager</li>
                        </ul>
                    </div>
                    ...
                    <div class="row-fluid">
                        <div class="span4">
                            <div class="admin-card">
                                <h4 style="margin-top:0;"><?= $editRow ? 'Edit Transmission' : 'Add New Transmission' ?></h4>
                                ...
                                <form method="post">
                                    ...
                                    <button type="submit" name="<?= $editRow ? 'lookup_update_btn' : 'lookup_add_btn' ?>" class="btn btn-admin-primary">
                                        <?= $editRow ? 'Update' : 'Add Transmission' ?>
                                    </button>
                                    <?php if ($editRow): ?>
                                    <a href="transmission_manager.php" class="btn">Cancel</a>
                                    <?php endif; ?>
                                </form>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <form method="post" style="display:inline" onsubmit="return confirm('Delete this transmission?')">
